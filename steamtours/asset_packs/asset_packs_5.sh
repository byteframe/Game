...
MAKE_ASSET_PACK()
{
  ...
  if [ ${A} == "${B}" ] || [[ -z ${B} && ! -d "${C}"/${A} && ! -d "${G}"/${A} ]]; then
    ...
    if [ -z ${INITIALIZE_ONLY} ]; then
      ...
      for FILE in $(find ${A}/materials/ -iname "*.vmt"); do sed -i -e "/\[\$[xX]360\]/d" -e "s/\s*\[\$WIN32\]//" -e s:$'\x93':\":g -e s:$'\x94':\":g -e "s:\\\:/:g" ${FILE}; done
      ...
      header ${A}" | RUNNING CONVERSION SCRIPTS AND BLACKLISTING MODEL..."
      ...
      header ${A}" | BLACKLISTING VMDLS AND CHECKING VMATS..."
      ...
      header ${A}" | BUILDING GAME FILES THEN ERASING SOUNDS..."
      cat "${G}"/../steamtours/gameinfo.gi | grep asset_pack ; sleep 10
      for TYPE in sounds scenes materials particles models soundevents; do
        while ! grep -q "compiled," ${A}/${A}_build_${TYPE}.log 2> /dev/null; do
          ...
          "${G}"/../bin/win64/resourcecompiler.exe -nominidumps -v -r -i "C:\Program Files (x86)\Steam\steamapps\common\SteamVR\tools\steamvr_environments\content\steamtours_addons\\\\${A}\\\\${TYPE}\*" >> ${A}/${A}_build_${TYPE}.log
        done
        cat ${A}/${A}.txt ${A}/${A}_build_${TYPE}.log | sed "/Failed to find compiler/d" > ${A}/_${A}.txt
...
##------------------------------------------------------------------------------ ep2
PRE_IMPORT_FIXES() { sed -i -e "s:skybox:nature:" ${A}/materials/nature/mountainscape.vmt ${A}/materials/nature/vista_card_12.vmt; }
...
##------------------------------------------------------------------------------ pm
PRE_IMPORT_FIXES() { sed -i -e "s/.5\`/.5/" ${A}/scripts/game_sounds_weapons.txt; }
...
##------------------------------------------------------------------------------ revelations
PRE_IMPORT_FIXES() { mv -v ${A}/sounds/Revelations/Greg/Kulkukan_swing.wav ${A}/_cull 2> /dev/null; }
...
##------------------------------------------------------------------------------ se1
PRE_IMPORT_FIXES() { rm -fvr ${A}/materials/models/hl2_materials ${A}/models/hl2_props; }
...
##------------------------------------------------------------------------------ nucleardawn
PRE_IMPORT_FIXES() { sed -i -e "s:\t\"\$detailscale:\t//\"\$detailscale:" ${A}/materials/models/rts_structures/rts_armoury/rts_armoury_ct_displays.vmt ${A}/materials/models/rts_structures/rts_armoury/rts_armoury_emp_displays.vmt; }
...
##------------------------------------------------------------------------------ p3 TODO: some weird space fix thing with cars/chevrolet
PRE_IMPORT_FIXES() { find asset_pack_p3/materials/ -name *.vmt | while read FILE; do convert_to_utf8 ${FILE}; done; }
...
##------------------------------------------------------------------------------ tacint
PRE_IMPORT_FIXES() { mv -v ${A}/particles/ti_explodefx_props.pcf ${A}/_cull 2> /dev/null; }
...
##------------------------------------------------------------------------------ bms
PRE_IMPORT_FIXES() { convert_to_utf8 ${A}/materials/models/xenians/houndeye/houndeye.vmt; }
...
##------------------------------------------------------------------------------ portal_stories
PRE_IMPORT_FIXES() { mv -v ${A}/scripts/game_sounds_ps_virgil_autogenerated.txt ${A}/_cull 2> /dev/null; }
MAKE_ASSET_PACK "${S}/Portal Stories Mel/portal_stories" asset_pack_portal2 "pak01_dir.vpk"
##------------------------------------------------------------------------------ aperturetag
MAKE_ASSET_PACK "${S}/Aperture Tag/aperturetag" asset_pack_portal2 "pak01_dir.vpk" "pak02_dir.vpk"
##------------------------------------------------------------------------------ infra
PRE_IMPORT_FIXES() { mv -v ${A}/sounds/world/plane_overpass.wav ${A}/_cull 2> /dev/null; }
