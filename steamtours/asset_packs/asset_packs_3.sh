...
convert_to_utf8()
{
  iconv -f WINDOWS-1250 -t UTF-8 ${1} > temp.out
  mv -v temp.out ${1}
}
MAKE_ASSET_PACK()
{
  ...
  if [ ${A} == "${B}" ] || [[ -z ${B} && ! -d "${C}"/${A} && ! -d "${G}"/${A} ]]; then
    ...
    if [ -z ${INITIALIZE_ONLY} ]; then
      header "${A} | PREPPING SOURCE FILES {!}" | tee -a ${A}/${A}.txt
      {
        mv ${A}/sound/* ${A}/sounds
        ...
        pause
        python3 -m no_vtf -l tga -h pfm "${C}"/${A}/materials 1> /dev/null 2>> ${A}/${A}.txt
      ...
      for TYPE in sounds scenes materials particles models soundevents; do
        unset PAUSE
        while [ -z ${PAUSE} ]; do
          "${G}"/../bin/win64/resourcecompiler.exe -r -i "C:\Program Files (x86)\Steam\steamapps\common\SteamVR\tools\steamvr_environments\content\steamtours_addons\\\\${A}\\\\${TYPE}\*" >> ${A}/${A}.txt
      ...
      header "${A} | DELETING VTF AND GAME CACHE BINS..."
...
PRE_IMPORT_FIXES()
{
  cp -R black.vtf white.vtf ${A}/materials/
...
MAKE_ASSET_PACK "${D}/Contagion/contagion" none "../vpks/materials_dir.vpk" "../vpks/models_dir.vpk" "../vpks/scripts_dir.vpk"
##------------------------------------------------------------------------------ csgo
PRE_IMPORT_FIXES()
{
  cp -R black.vtf white.vtf ${A}/materials/
  ...
  convert_to_utf8 ${A}/scripts/soundscapes_militia.vsc
}
MODELS="weapons/v_rif_m4a1_s player/custom_player/legacy/ctm_diver_varianta"
MAKE_ASSET_PACK "${D}/Counter-Strike Global Offensive/csgo" none "pak01_dir.vpk"
##------------------------------------------------------------------------------ dearesther
# [lostbeach/objects]/candle_001-003.vmat proxies black
...
MAKE_ASSET_PACK "${E}/insurgency2/insurgency" none "insurgency_misc_dir.vpk" "insurgency_materials_dir.vpk" "insurgency_models_dir.vpk" "insurgency_particles_dir.vpk" "insurgency_sound_dir.vpk"
##------------------------------------------------------------------------------ ship
PRE_IMPORT_FIXES()
{
  convert_to_utf8 ${A}/scripts/soundscapes_ship_christmas.txt
}
...
PRE_IMPORT_FIXES()
{
  ...
  find ${A}/scripts -name "game_sounds_*.txt" | while read FILE; do convert_to_utf8 ${FILE}; done
}
...
MAKE_ASSET_PACK "${D}/Left 4 Dead 2/left4dead2" none "pak01_dir.vpk" "../left4dead2_dlc1/pak01_dir.vpk" "../left4dead2_dlc2/pak01_dir.vpk" "../left4dead2_dlc3/pak01_dir.vpk"  "../other_loose_files.vpk"
##------------------------------------------------------------------------------ portal2
PRE_IMPORT_FIXES()
{
...
MAKE_ASSET_PACK "${E}/Portal 2/portal2" none "pak01_dir.vpk" "../portal2_dlc1/pak01_dir.vpk" "../portal2_dlc2/pak01_dir.vpk" "../other_loose_files_in_dlc_folders.vpk"
##------------------------------------------------------------------------------ portal_stories
MAKE_ASSET_PACK "${E}/Portal Stories Mel/portal_stories" asset_pack_portal2 "pak01_dir.vpk"
##------------------------------------------------------------------------------ hl2 ( VPK_LINK_FIX:  hl2_sound_vo__00[0-4].vpk << hl2_sound_vo_english_00[0-4].vpk )
...
PRE_IMPORT_FIXES()
{
  ...
  convert_to_utf8 ${A}/scripts/game_sounds_phonebook.txt
}
...
PRE_IMPORT_FIXES()
{
  ...
  convert_to_utf8 ${A}/scripts/game_sounds_vehicles.txt
}
MAKE_ASSET_PACK "${E}/Nuclear Dawn/nucleardawn" asset_pack_hl2 "pak01_dir.vpk"
##------------------------------------------------------------------------------ gstringv2
PRE_IMPORT_FIXES()
{
  #### orbit_sky_cube.tgaOFF THIS MIGHT HAVE GOTTEN GENERATED BY SCRIPT
...
MAKE_ASSET_PACK "${D}/G String/gstringv2" asset_pack_hl2 "gstringv2_dir.vpk"
##------------------------------------------------------------------------------ episodic
...
PRE_IMPORT_FIXES()
{
  convert_to_utf8 ${A}/scripts/soundscapes_militia.txt
...
MAKE_ASSET_PACK "${D}/Double Action/dab" asset_pack_hl2
##------------------------------------------------------------------------------zps
PRE_IMPORT_FIXES()
{
...
MAKE_ASSET_PACK "${D}/Zombie Panic Source/zps" asset_pack_hl2 "../vpks/zps_content_dir.vpk"
##------------------------------------------------------------------------------ nmrih
PRE_IMPORT_FIXES()
{
  mv -v ${A}/particles/nmrih_molotov.pcf ${A}/particles/nmrih_molotov._pcf 2> /dev/null
  sed -i -e "s:\\\:/:g" -e "s:Wally head :Wally_head_:gi"  ${A}/materials/models/player/wally/nmrih\ wally\ glasses.vmt
  sed -i -e "s:\\\:/:g" ${A}/materials/models/nmr_zombie/julie/body\ diffuse\ z.vmt
  sed -i -e "s:\\\:/:g" ${A}/materials/models/nmr_zombie/julie/head\ diffuse\ z.vmt
  sed -i -e "s:\\\:/:g" -e "s:Wally body normal:Wally_body_normal:gi" ${A}/materials/models/player/wally/wally\ body\ diffuse.vmt
  sed -i -e "s:\\\:/:g" ${A}/materials/models/player/wally/wally\ head\ diffuse.vmt
  sed -i -e "s:\\\:/:g" ${A}/materials/models/player/wally/wally\ head\ diffuse_zom.vmt
  sed -i -e "s:\\\:/:g" ${A}/materials/models/player/wally/wally\ body\ diffuse_zom.vmt
...
MAKE_ASSET_PACK "${D}/nmrih/nmrih" asset_pack_hl2 "nmrih_dir.vpk"
##------------------------------------------------------------------------------ swarm
...
MAKE_ASSET_PACK "${D}/Alien Swarm/swarm" asset_pack_hl2 "pak01_dir.vpk"
##------------------------------------------------------------------------------ bms ( VPK_LINK_FIX: bms_sound_vo__00[0-3].vpk << bms_sound_vo_english_00[0-3].vpk )
PRE_IMPORT_FIXES()
{
...
MAKE_ASSET_PACK "${D}/Black Mesa/bms" asset_pack_hl2 "bms_materials_dir.vpk" "bms_models_dir.vpk" "bms_textures_dir.vpk" "bms_sounds_misc_dir.vpk" "bms_sound_vo_english_dir.vpk" "bms_misc_dir.vpk"
##------------------------------------------------------------------------------ vampire
...
PRE_IMPORT_FIXES()
{
  ...
  find asset_pack_p3/scripts/ -name *.vmt | while read FILE; do convert_to_utf8 ${FILE}; done
}
MODELS="characters"
...
MODELS="${MODELS} small_things/cow_pad small_things/drocha_01"
MODELS="${MODELS} small_things/office_papers_01_d_damage small_things/office_papers_01_i_damage small_things/office_papers_01_j_damage test/testHouse"
MODELS="${MODELS} weapons/bat/bat_rotate_flight weapons/bat/dat_rotate_flight"
MAKE_ASSET_PACK "${D}/Postal III/p3" none "p3_english_stuff.vpk"
##------------------------------------------------------------------------------ zenozoik
MAKE_ASSET_PACK "${D}/ZenoClash/zenozoik" none
##------------------------------------------------------------------------------ garrysmod
MODELS="h11plr player/hostage"
MAKE_ASSET_PACK "${D}/GarrysMod/garrysmod" none "fallbacks_dir.vpk" "garrysmod_dir.vpk"
##------------------------------------------------------------------------------ eye
PRE_IMPORT_FIXES()
{
  mv -v ${A}/materials/vgui/equipment/vgui_weapondefault_.vmt ${A}/materials/vgui/equipment/vgui_weapondefault.vmt 2> /dev/null
  convert_to_utf8 ${A}/scripts/soundscapes_new_eden.txt
  convert_to_utf8 ${A}/materials/models/arches/candle_flame.vmt
  mv -v ${A}/scripts/game_sounds_reloads.txt ${A}/_cull
}
MAKE_ASSET_PACK "${D}/EYE/eye" none
##------------------------------------------------------------------------------