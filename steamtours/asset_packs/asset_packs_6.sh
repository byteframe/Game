...
##------------------------------------------------------------------------------ esmod
PRE_IMPORT_FIXES() { convert_to_utf8 ${A}/materials/models/props_es/terminals/monitor.vmt; }
MODELS="capships/gm_destroyer_test"
MAKE_ASSET_PACK "${S}/Source SDK Base/esmod"
##------------------------------------------------------------------------------ estrangedact1
PRE_IMPORT_FIXES() { convert_to_utf8 ${A}/materials/models/www.gaming-models.de/baustellenset/baustellenbarke.vmt; }
...
##------------------------------------------------------------------------------
PRE_IMPORT_FIXES() { mv -v ${A}/sounds/Zenlil_1/iss_sound/tree_group_45/"Pawn44+45_1-2-3-2+-0.wav" ${A}/_cull 2> /dev/null; }
...
##------------------------------------------------------------------------------ p3
PRE_IMPORT_FIXES() { find ${A}/materials/ -name *.vmt | while read FILE; do convert_to_utf8 ${FILE}; done; }
...
################################################################################ VAMPIRE XXX ttz/h smd+qc files erased, still version 2
...
################################################################################ TF XXX models seemingly have to be done by hand cause of 260 character limit
PRE_IMPORT_FIXES() {
...
#needed HFSExtract to work
#some script culls, and stuff in armor and monster (kobald and vampire directory)
